...
public class InteractiveRobotAgent : Agent
{
    ...
    private void SetupMagnetCollider() // in case I forgot
    {
        ...
        InteractiveMagnetTrigger trigger = magnet.GetComponent<InteractiveMagnetTrigger>();
        if (trigger == null)
        {
            trigger = magnet.gameObject.AddComponent<InteractiveMagnetTrigger>();
        }
    ...
    public override void OnEpisodeBegin() // reset positions, states, and curriculum progression
    {
        ...
        previousDistanceToTarget = movableBox != null 
            ? Vector3.Distance(new Vector3(movableBox.transform.position.x, 0, movableBox.transform.position.z), new Vector3(targetPosition.x, 0, targetPosition.z)) 
            : Vector3.Distance(new Vector3(boxStartPosition.x, 0, boxStartPosition.z), new Vector3(targetPosition.x, 0, targetPosition.z));
            
        previousHeightError = movableBox != null
            ? Mathf.Abs((movableBox.transform.position.y - targetPosition.y) - 0.1f)
            : Mathf.Abs((boxStartPosition.y - targetPosition.y) - 0.1f);
    ...
    private void CalculateRewards(float energyThisStep)
    {
        ...
        else
        {
            ...
            // --- NEW: Height shaping reward when positioned over target zone ---
            float currentHeightError = Mathf.Abs((boxPos.y - targetPosition.y) - 0.1f);
            
            // Only reward precise lowering if the box is horizontally close to the target
            if (distanceToTarget < 0.3f)
            {
                float heightImprovement = previousHeightError - currentHeightError;
                // Reward moving towards the ideal placement height (0.1m above target)
                AddReward(Mathf.Max(0, heightImprovement) * 3f * rewardMultiplier);
                
                // Add a small dense reward for holding steady at the correct drop height
                if (currentHeightError < 0.05f)
                {
    ...
    private void FixedUpdate()
    {
        ...
        else
        {
            ...
            float verticalDiff = boxPos.y - targetPosition.y;
            distanceToTarget = realHorizontalDistance;

            // Auto-detach when box (carried) reaches zone B — let it land into the trigger
            // Horizontal threshold forces dead-center placement before dropping
            // Vertical threshold ensures it drops from right above the surface (0 to 0.15m), without pushing into ground
            if (realHorizontalDistance < 0.1f && verticalDiff >= -0.05f && verticalDiff <= 0.15f)
            {
                Debug.Log($"<color=yellow>[AUTO-DETACH] Box correctly placed (XZ: {realHorizontalDistance:F3}m, Y diff: {verticalDiff:F3}m), releasing...</color>");
                DetachBox();
                AddReward(5f * rewardMultiplier);  // Boosted intermediate reward for careful clean delivery
            }
        }
    }
    ...
    private void CollectEpisodeData(bool success)
    {
        ...
        {
            PerformanceTracker.Instance.RecordEpisode((RobotAgent)(Agent)this, success, timeTaken, totalEnergyConsumed, accuracy);
        }
    }
    ...
    void OnGUI() // generate simple on-screen HUD for debugging and performance tracking
    {
        ...
        else if (PerformanceTracker.Instance != null && !PerformanceTracker.Instance.IsBestPerformer((RobotAgent)(Agent)this))
        {
